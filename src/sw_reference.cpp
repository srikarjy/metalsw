...
TracebackResult smithWatermanTraceback(const std::string &query,
    ...
    for (size_t i = 1; i <= n; ++i) {
        H[i][0] = 0; E[i][0] = 0; F[i][0] = 0; trace[i][0] = 0;
        for (size_t j = 1; j <= m; ++j) {
            ...
            if (h < eVal) { h = eVal; dir = 3; }  // left (E)
            if (h < fVal) { h = fVal; dir = 2; }  // up (F)
            if (h < 0) { h = 0; dir = 0; }        // stop
    ...
    while (i > 0 && j > 0 && H[i][j] > 0) {
        ...
        if (dir == 1) {  // diagonal (match)
            aligned_query.push_back(query[i - 1]);
            aligned_target.push_back(target[j - 1]);
            --i; --j;
        } else if (dir == 2) {  // up (gap in query / deletion in target)
            aligned_query.push_back('-');
            aligned_target.push_back(target[j - 1]);
            --i;
        } else if (dir == 3) {  // left (gap in target / insertion in query)
            aligned_query.push_back(query[i - 1]);
    ...
    return {aligned_query, aligned_target, H[best_i][best_j],
            static_cast<int>(query_start), static_cast<int>(query_end),
            static_cast<int>(target_start), static_cast<int>(target_end)};
}
