...
- [ ] Power measurement via `powermetrics` sampled during runs — script exists (`scripts/run_thermal_power_protocol.sh`) but not yet executed (needs sudo).
...
**Honest finding, not yet explained away:** Parasail's scalar CPU baseline currently beats the metalsw GPU kernel on this corpus. Two likely causes, neither yet fixed: (1) `GpuRunner::run()` allocates and releases all Metal buffers on every call instead of reusing them across iterations, so buffer setup/teardown cost is paid every measured iteration; (2) 750 sequences (one GPU thread each) may not be enough parallel work to saturate the M2's 10 GPU cores, especially with dispatch/readback overhead per call. Also notable: the int8 fast path had 0/750 sequences overflow on this real corpus — every sequence fell through to the int16 kernel, meaning the int8 optimization currently contributes nothing for a hemoglobin-vs-Swiss-Prot workload (scores routinely exceed 127). Sequence-length binning, buffer reuse across bench iterations, and/or a larger corpus are the next things to try before this can go in a benchmark table as a real comparison.
