...
int main(int argc, char **argv) {
    ...
    std::fprintf(stderr, "warm-up: %d iterations (discarded, settles thermal/clock state)\n",
                 kWarmupIterations);
    ...
    for (int i = 0; i < kWarmupIterations; ++i) {
        ...
        if (i == 0) coldStartGcups = gcups;  // first iteration = cold start
    }
    ...
    auto report = [&](std::ostream &os) {
        ...
        os << "corpus: " << dbPath << " (" << dbRecords.size() << " sequences, "
           << totalDbResidues << " total residues)\n";
