...
main(int argc, char **argv)
{
    ...
    {
        std::fprintf(
            stderr, "usage: %s <query.fasta> <db.fasta> [topN] [metallib_path]\n", argv[0]);
        return 1;
    }
    ...
    metalsw::Timer     timer;
    std::vector<int>   gpuScores = runner.runWavefront(query, dbRecords, gapOpen, gapExtend);
    ...
    for (size_t i = 0; i < dbRecords.size(); ++i)
    {
        ...
        const int   oracleScore =
            metalsw::smithWatermanScore(query, rec.sequence, gapOpen, gapExtend);
        const bool pass = oracleScore == gpuScores[i];
        std::fprintf(stderr,
                     "%-20s gpu=%-6d oracle=%-6d %s\n",
                     rec.id.c_str(),
                     gpuScores[i],
                     oracleScore,
                     pass ? "PASS" : "FAIL");
